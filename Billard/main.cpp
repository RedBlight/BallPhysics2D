...
{
	sf::Texture tex_ball;
	if(!tex_ball.loadFromFile("resources/ball.png")) return 0;

	//for(int i=0; i<2000; ++i)Physics::AddObj( 960, 25, 1, 10, 250-(rand()%500), rand()%500 );

	Physics::Init();
	rand();
	//for(int i=0; i<100; ++i) Physics::AddObj( rand()%1920, rand()%1080, rand()%20, rand()%50, rand()%500, rand()%500, &tex_ball );
	for(int i=0; i<10; ++i)Physics::AddObj( 980-rand()%40, 560-rand()%40, 19, 25, 500-rand()%1000, 500-rand()%1000, &tex_ball );


	/* Billard
	Physics::AddObj( 400, 500, 4, 30, 0, 0, &tex_ball );

	Physics::AddObj( 1000, 500, 4, 30, 0, 0, &tex_ball );

	Physics::AddObj( 1052, 469, 4, 30, 0, 0, &tex_ball );
	Physics::AddObj( 1052, 531, 4, 30, 0, 0, &tex_ball );

	Physics::AddObj( 1104, 439, 4, 30, 0, 0, &tex_ball );
	Physics::AddObj( 1104, 500, 4, 30, 0, 0, &tex_ball );
	Physics::AddObj( 1104, 561, 4, 30, 0, 0, &tex_ball );

	Physics::AddObj( 1156, 408, 4, 30, 0, 0, &tex_ball );
	Physics::AddObj( 1156, 469, 4, 30, 0, 0, &tex_ball );
	Physics::AddObj( 1156, 531, 4, 30, 0, 0, &tex_ball );
	Physics::AddObj( 1156, 592, 4, 30, 0, 0, &tex_ball );
	*/
    clk.restart();
    ...
    while( running )
    {
        ...
        while (wnd.pollEvent(event))
        {
            ...
            if( Keyboard::isKeyPressed(Keyboard::W) ) for(int i=0; i<10; ++i)Physics::AddObj( 980-rand()%40, 560-rand()%40, 19, 25, 500-rand()%1000, 500-rand()%1000, &tex_ball );
		...
		time_elapsed = (float)clk.getElapsedTime().asMicroseconds();
