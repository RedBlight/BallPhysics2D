...
namespace Physics
{
	...
	// Functions
	void Init( );
	void AddObj( double, double, double, double, double, double );
	void GrabAt( );
	void Push( );
	void Run( double );
...
void Physics::AddObj( double x, double y, double d, double r, double v_x, double v_y )
{
	...
	//Physics::shp[ Physics::obj_count ].setFillColor( Color( 150, 150, 150) );
...
void Physics::Run( double _dt )
{
	...
	for(int i=0; i<obj_count; ++i)
	{
		//v_x[i] *= 1 - (0.4 * time_factor);
		//v_y[i] *= 1 - (0.4 * time_factor);
